 ...
 */
export async function getPlausibleComparison(period: string = "30d"): Promise<Record<string, { value: number; change_percent: number; comparison_value: number; change: number }> | null> {
  const apiKey = process.env.PLAUSIBLE_API_KEY;
